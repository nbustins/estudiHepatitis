...
dattrs <- c("Class","AGE","SEX","STEROID","ANTIVIRALS","FATIGUE","MALAISE","ANOREXIA","LIVER_BIG",
            "LIVER_FIRM","SPLEEN_PAL","SPIDERS","ASCITES","VARICES","BILIRUBIN","ALK_PHOSPHATE",
...
```{r}
quantattrs <- c("AGE","BILIRUBIN","ALK_PHOSPHATE","SGOT","ALBUMIN","PROTIME")
catattrs <- c("Class","SEX","STEROID","ANTIVIRALS","FATIGUE","MALAISE","ANOREXIA","LIVER_BIG",
              "LIVER_FIRM","SPLEEN_PAL","SPIDERS","ASCITES","VARICES","HISTOLOGY")
```
...
```{r}
ggplot(data = data_train, aes(x = Class)) + geom_bar()
ggplot(data = data_test, aes(x = Class)) + geom_bar()
```
...
Podem veure com utilitzant simplement els atributs quantitatius obtenim un model bastant bo si ens guiem per l'AUC.
```{r}
...
summary(log.qual)
```
Sembla ser que també obtenim un bon model amb aquestes variables.
```{r}
...
Obtenim uns molt bons resultats, l'AUC s'ha mantingut molt elevat i hem reduït el model utilitzant només dues variables. Naturalment l'AUC ha disminuit, ja que les altres variables aportaven una mica d'informació encara que no fos extremadament significativa però amb aquesta versió reduïda hem aconseguit un model molt més petit que segueix sent bo.
```{r}
...
En aquest últim model utilitzarem les variables tant qualitatives com quantitatives que hem vist que tenien més efecte en els models. Veiem que totes les variables aporten, segons el test de Wald els coeficients que obtenim són significativament diferents de 0.
```{r}
...
En aquest apartat farem una comparativa dels models des de el punt de vista de sensibilitat, especificitat i exactitud. 
```{r}
...
for (model in models){
  mat <- rbind(mat,
               coords(model, x = "best", 
                      ret = c("specificity","sensitivity","accuracy","acuracy"), transpose = F)
               )
}
